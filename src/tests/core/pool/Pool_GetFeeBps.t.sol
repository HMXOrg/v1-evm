...
import { Pool_BaseTest, console, PoolConfig, PoolMath } from "./Pool_BaseTest.t.sol";

contract Pool_GetFeeBpsTest is Pool_BaseTest {
  ...
  function testCorrectness_GetFeeBps() external {
    ...
    assertEq(
      pool.poolMath().getAddLiquidityFeeBps(pool, address(matic), 1000),
      100
    );
    assertEq(
      pool.poolMath().getAddLiquidityFeeBps(pool, address(matic), 5000),
      104
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 1000),
      100
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 5000),
      104
    );
    ...
    assertEq(
      pool.poolMath().getAddLiquidityFeeBps(pool, address(matic), 10000),
      150
    );
    assertEq(
      pool.poolMath().getAddLiquidityFeeBps(pool, address(matic), 20000),
      150
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 1000),
    ...
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 10000),
      50
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 20000),
      50
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 25000),
      50
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 100000),
      150
    );
    ...
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 50000),
      28
    );
    assertEq(
      pool.poolMath().getRemoveLiquidityFeeBps(pool, address(matic), 80000),
      28
    );
